...
# #     psi_x_pred = torch.autograd.grad(psi_pred, bc.x_BC_2, grad_outputs=torch.ones_like(psi_pred), create_graph=True)[0]
# #     psi_xx_pred = torch.autograd.grad(psi_x_pred, bc.x_BC_2, grad_outputs=torch.ones_like(psi_x_pred), create_graph=True)[0]
...
#     psi_x_end_pred = torch.autograd.grad(psi_end_pred, bc.x_BC_2, grad_outputs=torch.ones_like(psi_end_pred), create_graph=True)[0]
#     psi_xx_end_pred = torch.autograd.grad(psi_x_end_pred, bc.x_BC_2, grad_outputs=torch.ones_like(psi_x_end_pred), create_graph=True)[0]
    
#     # Calculate boundary condition loss
#     mse_bc = (torch.mean((bc.w_train - w_pred)**2) +
#               torch.mean((bc.psi_train - psi_pred)**2) +
#               torch.mean((bc.psi_x_train - psi_x_end_pred)**2) +
#               torch.mean((bc.psi_xx_train - psi_xx_end_pred)**2))
...
#         loss.backward(retain_graph=True)  # Ensure the graph is retained
#         optimizer_w.step()
#         optimizer_psi.step()
...
#     N_u = 25000*mult_constant
#     N_f = 60000*mult_constant
...
#     psi_xx_train = torch.ones((N_u, 1), device=device, dtype=torch.float32) * (F)
